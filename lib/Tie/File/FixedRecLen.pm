...
Apart from the configuration parameters mentioned below, you should use this
module in just the same way as L<Tie::File>.
...
Do B<not> try using cacheing or deferred writing, at least not yet. The
current version of Tie::File is not easily overridden, and to save effort the
cache and deferred writing features have not been looked at for
Tie::File::FixedRecLen. Furthermore, their tests actually B<fail> so that's a
sure sign they are really broken right now in this module.
...
In Tie::File::FixedRecLen this is a really stupid thing to do, so please
